...
// TwoWayStrategy is a Strategy variant that also allows callers to buy repoTokens
// from the vault via buyRepoToken, priced at oracle rate minus discountRateBuyMarkup.
...
contract TwoWayStrategy is BaseStrategy, Pausable, AccessControl {
    using SafeERC20 for IERC20;
    using RepoTokenList for RepoTokenListData;
    using TermAuctionList for TermAuctionListData;

    /**
     * @notice Constructor to initialize the TwoWayStrategy contract
     ...
     * @param _discountRateMarkup The markup added to the oracle rate in sellRepoToken
     * @param _discountRateBuyMarkup The markup subtracted from the oracle rate in buyRepoToken
     */
    struct StrategyParams {
        ...
        uint256 _discountRateBuyMarkup;
    }
    ...
    error OfferNotFound();
    error InsufficientRepoTokenBalance(uint256 have, uint256 want);
    ...
    /**
     * @notice Set the markup subtracted from the oracle rate when the vault sells repoTokens
     * @param newDiscountRateBuyMarkup The new buy-side discount rate markup
     */
    function setDiscountRateBuyMarkup(
        uint256 newDiscountRateBuyMarkup
    ) external onlyRole(GOVERNOR_ROLE) {
        TERM_VAULT_EVENT_EMITTER.emitDiscountRateBuyMarkupUpdated(
            strategyState.discountRateBuyMarkup,
            newDiscountRateBuyMarkup
        );
        strategyState.discountRateBuyMarkup = newDiscountRateBuyMarkup;
    }
    ...
    /**
     * @notice Allows the purchase of a specified amount of a repoToken held by the strategy.
     * @param repoToken The address of the repoToken to be bought.
     * @param repoTokenAmount The amount of the repoToken to be bought.
     *
     * @dev Reverse of {sellRepoToken}: the caller pays assets and receives repoTokens.
     * Pricing uses the oracle discount rate minus `discountRateBuyMarkup` (floored at 0).
     */
    function buyRepoToken(
        address repoToken,
        uint256 repoTokenAmount
        ...
        uint256 held = IERC20(repoToken).balanceOf(address(this));
        if (held < repoTokenAmount) {
            revert InsufficientRepoTokenBalance(held, repoTokenAmount);
        }
        ...
        IERC20(repoToken).safeTransfer(msg.sender, repoTokenAmount);
    ...
    constructor(
        ...
        strategyState = StrategyState({
            ...
            discountRateBuyMarkup: _params._discountRateBuyMarkup,
